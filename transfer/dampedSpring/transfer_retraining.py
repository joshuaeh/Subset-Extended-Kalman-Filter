...
# constants
results_dir = os.path.join(DATA_DIR, "transfer_retraining")
...
if __name__ == "__main__":
    for scenario in SCENARIOS:
        ...
        if not (
            os.path.exists(os.path.join(results_dir, f"{scenario_name}_model_weights.pth")) and
            os.path.exists(os.path.join(results_dir, f"{scenario_name}_bestResult_metrics.csv")) and
            os.path.exists(os.path.join(results_dir, f"{scenario_name}_allTrials_metrics.csv"))
        ):
            ...
            tuner = tune.Tuner(
                ...
                run_config = tune.RunConfig(
                    verbose=1,
                    name=f"dampedSpring_transfer_retraining_{scenario_name}",
