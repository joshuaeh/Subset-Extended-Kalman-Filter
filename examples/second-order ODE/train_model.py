...
def main():
    ...
    EPSILON = config["TRAINING_EPSILON"]
    TRAINING_SAMPLES = config["TRAINING_SAMPLES"]
    TESTING_SAMPLES = config["TESTING_SAMPLES"]
    TRAINING_EPOCHS = config["TRAINING_EPOCHS"]
    MODEL_SAVE_PATH = config["TRAINING_MODEL_SAVE_PATH"]
    TRAINING_DATA_SAVE_PATH = config["TRAINING_DATA_SAVE_PATH"]
    PLOT_PATH = config["TRAINING_PLOT_PATH"]
    ...
    for epoch in range(TRAINING_EPOCHS):
        ...
        if epoch % 100 == 0:
            print(f"Epoch {epoch}, Train Loss: {loss.item()}, Test Loss: {test_loss.item()}", end="\r")
